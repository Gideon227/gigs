...
const Card = ({ title, text, image, width, imageHeight, image2, width2, height2, style2, extraImage = false, background, height, extraStyles }: CardProps) => {
  return (
    <div className={`rounded-[16px] py-8 lg:pl-8 lg:min-w-[600px] relative ${height} space-y-4 ${background} ${extraStyles} max-lg:w-[370px] max-lg:pl-4 max-lg:py-6 max-lg:space-y-2`}>
        <h1 className='font-semibold text-[#FCFCFC] text-[26px] leading-[31.2px] max-lg:text-[21px]'>{title}</h1>
        <p className='text-[14px] text-[#CACACA] leading-[24px] lg:w-2/3 items-start max-lg:leading-[21px] break-words max-lg:pr-20'>{text}</p>
        ...
        <div className='absolute bottom-0 right-0 z-10'>
            ...
            <Image src={image} alt='image' width={90} height={imageHeight} className='lg:hidden rounded-br-2xl' />
