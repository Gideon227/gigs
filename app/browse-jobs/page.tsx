          ...
          <div className='browse-jobs_bg w-full lg:h-[606px] max-lg:h-[350px] flex justify-center lg:px-20 max-lg:px-8 max-sm:px-4 lg:space-x-10 max-lg:space-x-4 max-sm:space-x-0'>
            <div className='max-w-[1600px] flex-1 flex items-center justify-center max-md:flex-col'>
              <div className='md:w-2/3 max-sm:space-y-0 space-y-2 max-lg:space-y-3 lg:pr-6'>
                ...
                <h1 className='font-semibold text-[40px] lg:leading-[60px] text-heading max-lg:text-[27px] max-lg:leading-[32px] max-sm:text-[24px] max-sm:leading-8'> Explore Microsoft Power Platform & Dynamics 365 Jobs</h1>
                      
                <p className='text-heading text-[16px] leading-[27px] max-lg:leading-[25px] max-lg:text-[15px] max-sm:text-[13px] max-sm:leading-6 max-sm:pt-4 md:pr-6'>GIGS.TECH is a hyper-focused job aggregation platform built from the ground up to serve<br className='max-md:hidden'/> one mission: Help Microsoft job seekers discover the freshest roles, and help recruiters<br className='max-md:hidden'/> connect with qualified candidates — faster and smarter. </p>
              </div>
              <div className='md:w-1/3 flex items-end justify-center flex-1 max-md:hidden'>
                <Image src='/browse-jobs_hero.svg' alt='About us page illustration' priority width={585} height={514} className='object-contain'/>
              </div>  
            </div>
          </div>
