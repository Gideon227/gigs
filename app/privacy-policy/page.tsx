...
const PrivacyPage = () => {
  ...
  return (
    <div className='overflow-hidden'>
        <div className='hero_bg w-full h-[525px] bg-bottom -mt-20 flex items-center justify-center'>
            <div className='lg:w-2/3 px-36 relative flex flex-col space-y-6 justify-center items-center text-center py-8'>
                <Image src='/Group 6.png' width={90} height={114} alt='sparkles' className='absolute left-0 top-0'/>

                <Image src='/Group 5.png' width={100} height={86} alt='sparkles' className='absolute right-0 bottom-0'/>

                <h1 className='leading-[84px] text-[56px] font-semibold text-heading'>Privacy Policy</h1>

                <p className='text-[16px] leading-[24px] font-normal text-heading'><b>Welcome to Gigs.Tech.</b> This Privacy Policy explains how we collect, use, and protect your information — whether you're a <b>job seeker</b> looking for your next Microsoft Business Apps role or a <b>recruiter</b> sourcing top Power Platform & Dynamics 365 talent.</p>
        ...
        <div className='bg-[#101217] lg:py-16 lg:px-20 px-6 py-10 flex h-screen '>
            <div className='w-1/3 bg-[#101217] p-4 border-r border-r-[#363636] h-full overflow-y-auto flex flex-col space-y-6 items-start max-lg:hidden'>
                <Link href='#data' onClick={() => scrollToSection(dataRef)} className={`pl-4 leading-[18px] ${activeId === 'data' ? 'text-primary border-l-2 border-l-primary text-[16px] font-semibold' : 'text-neutral text-[14px]'}`}>
                ...
                <Link 
                    ...
                    className={`pl-4 leading-[18px] ${activeId === 'information' ? 'text-primary border-l-2 border-l-primary text-[16px] font-semibold' : 'text-neutral text-[14px]'}`}>
                        How We Use Your Information
                </Link>

                <Link href='#cookies' onClick={() => scrollToSection(cookiesRef)} className={`pl-4 leading-[18px] ${activeId === 'cookies' ? 'text-primary border-l-2 border-l-primary text-[16px] font-semibold' : 'text-neutral text-[14px]'}`}>
                    Cookies and Tracking
                </Link>

                <Link href='#security' onClick={() => scrollToSection(securityRef)} className={`pl-4 leading-[18px] ${activeId === 'security' ? 'text-primary border-l-2 border-l-primary text-[16px] font-semibold' : 'text-neutral text-[14px]'}`}>
                    Data Sharing and Security  
                </Link>

                <Link href='#right' onClick={() => scrollToSection(rightRef)} className={`pl-4 leading-[18px] ${activeId === 'right' ? 'text-primary border-l-2 border-l-primary text-[16px] font-semibold' : 'text-neutral text-[14px]'}`}>
                    Your Rights
                </Link>

                <Link href='#policy' onClick={() => scrollToSection(policyRef)} className={`pl-4 leading-[18px] ${activeId === 'policy' ? 'text-primary border-l-2 border-l-primary text-[16px] font-semibold' : 'text-neutral text-[14px]'}`}>
            ...
            <main className='lg:w-2/3 lg:pl-12 overflow-y-auto h-full space-y-8 hide-scrollbar lg:pb-96' ref={containerRef}>
                ...
                >
                    ...
                    <p className='text-[14px] leading-[24px] text-heading'>Current <b>(MVP Stage)</b></p>
                    <br />
